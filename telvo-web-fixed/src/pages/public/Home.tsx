...
import heroWorkers from '@/assets/hero-workers.webp';
...
export function Home() {
  ...
  return (
    <div>
      {/* HERO */}
      <section className="relative overflow-hidden bg-gradient-to-r from-white via-white to-brand-900">
        <div className="container-page">
          <div className="grid lg:grid-cols-2 items-center gap-8 pt-14 pb-0 sm:pt-20 lg:pb-0">
            {/* Left: copy + search */}
            <div className="text-center lg:text-left py-6">
              ...
              <div className="mt-8 flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-3 animate-slide-up">
              ...
              {/* Trust chips, mirrors the app's own splash screen messaging */}
              <div className="mt-8 hidden sm:flex items-center justify-center lg:justify-start gap-6">
                {[
                  { icon: ShieldCheck, label: 'Verified Professionals' },
                  { icon: Clock, label: 'Fast & Reliable' },
                  { icon: ThumbsUp, label: 'Quality You Can Trust' },
                ].map((t) => (
                  <div key={t.label} className="flex items-center gap-2">
                    <span className="w-8 h-8 rounded-lg bg-brand-50 text-brand-600 flex items-center justify-center flex-shrink-0">
                      <t.icon size={15} />
                    </span>
                    <span className="text-xs font-medium text-ink-600 text-left">{t.label}</span>
                  </div>
                ))}
              </div>
            </div>

            {/* Right: worker photo, bleeds to the edge on large screens */}
            <div className="relative hidden lg:block h-[420px] xl:h-[480px]">
              <img
                src={heroWorkers}
                alt="Verified TELVO professionals: a cleaner, a mechanic, an electrician, a construction worker, and a tailor"
                className="absolute inset-y-0 right-[-2rem] xl:right-[-4rem] h-full w-auto max-w-none object-contain object-bottom"
              />
            </div>
          </div>

          {/* Search interface, full width, sits on the seam between the two panels */}
          <div className="relative z-10 pb-14 sm:pb-20 -mt-2 lg:-mt-10">
            <form
              onSubmit={handleSearch}
              className="max-w-2xl mx-auto lg:mx-0 bg-white rounded-2xl shadow-card-hover border border-ink-100 p-2 flex flex-col sm:flex-row gap-2 animate-scale-in"
            >
            ...
            <p className="mt-3 text-xs text-ink-400 text-center lg:text-left max-w-2xl mx-auto lg:mx-0">
