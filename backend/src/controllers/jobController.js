...
const acceptQuote = async (req, res) => {
  try {
    ...
    // 24h expiry: refuse acceptance on expired open jobs
    const created = job.createdAt ? new Date(job.createdAt) : null;
    const expiresAt = job.expiresAt
      ? new Date(job.expiresAt)
      : (created ? new Date(created.getTime() + 24 * 60 * 60 * 1000) : null);
    if (expiresAt && expiresAt.getTime() < Date.now() && ['posted', 'open', 'quotes_received', 'notified'].includes(String(job.status || '').toLowerCase())) {
      job.status = 'expired';
      await job.save();
      return errorResponse(res, 'This job has expired and can no longer accept quotes', 400);
    }
    if (job.acceptedQuoteId || String(job.status || '').toLowerCase() === 'accepted') {
      return errorResponse(res, 'A quote was already accepted for this job', 400);
    }
