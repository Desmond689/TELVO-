...
String getFriendlyErrorMessage(Object error, {String fallback = 'Something went wrong. Please try again.'}) {
  ...
  // Plain `Exception('some human sentence')` thrown deliberately by our own
  // code (e.g. upload validation: "too large", "unsupported file type",
  // "could not be found") doesn't match any technical pattern above. Prefer
  // that message over the generic fallback so the user sees what actually
  // went wrong instead of a vague "something went wrong". Only plain
  // Exceptions are eligible here — raw platform/technical exception types
  // (SocketException, FormatException, PlatformException, etc.) fall
  // through to [fallback] since their toString() isn't user-facing.
  final runtimeName = error.runtimeType.toString();
  if (runtimeName == '_Exception' || runtimeName == 'Exception') {
