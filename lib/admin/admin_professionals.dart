...
class _AdminProfessionalsScreenState extends State<AdminProfessionalsScreen> {
  ...
  List<UserModel> _filteredProfessionals(List<UserModel> professionals) {
    ...
    switch (_filterStatus) {
      case 'Verified':
        filtered = filtered.where((pro) => pro.verificationStatus == 'verified').toList();
        break;
      case 'Pending':
        filtered = filtered.where((pro) => pro.verificationStatus == 'pending').toList();
        break;
      case 'Rejected':
        filtered = filtered.where((pro) => pro.verificationStatus == 'rejected').toList();
        break;
      case 'Top Rated':
        filtered = filtered.where((pro) => (pro.rating ?? 0) >= 4.5).toList();
        break;
      case 'Suspended':
        filtered = filtered.where((pro) => pro.isSuspended).toList();
        break;
    }
        ...
        child: Row(
          children: [
            SafeAvatar(
              ...
              radius: 35,
              fallback: Text(
                (professional.fullName?.trim().isNotEmpty ?? false)
