...
class _ProfileSetupScreenState extends State<ProfileSetupScreen> {
  ...
  /// Treats null and empty/whitespace-only strings as "no photo" so we never
  /// hand an empty string to NetworkImage (which throws "Invalid image
  /// data").
  String? get _cleanProfilePhotoUrl {
                    ...
                    CircleAvatar(
                      ...
                      backgroundImage: _selectedPhotoPath != null
                          ? FileImage(File(_selectedPhotoPath!))
                                as ImageProvider
                          : (_cleanProfilePhotoUrl != null && !_photoLoadFailed
                                ? NetworkImage(_cleanProfilePhotoUrl!)
                                : null),
                      onBackgroundImageError:
                          (_selectedPhotoPath == null &&
                              _cleanProfilePhotoUrl != null)
                          ? (_, __) {
                              // Never call setState synchronously from an image
                              // error callback — it can fire mid-build/paint.
                              WidgetsBinding.instance.addPostFrameCallback((_) {
                                if (mounted && !_photoLoadFailed) {
