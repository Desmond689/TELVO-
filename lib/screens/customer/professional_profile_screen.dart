...
class _ProfessionalProfileScreenState extends State<ProfessionalProfileScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final iBlockedThem = me != null &&
        professional.id != null &&
        me.blockedUsers.contains(professional.id);
    final theyBlockedMe = me != null && professional.blockedUsers.contains(me.id);
        ...
        slivers: [
          SliverAppBar(
            ...
            actions: [
              ...
              IconButton(
                onPressed: () async {
                  final name = _professional?.fullName ?? 'this professional';
                  final cat = _professional?.category ?? 'services';
                  await Share.share(
                    'Check out $name on Telvo for $cat. Find trusted workers near you: https://telvo.app/download',
                    subject: 'Telvo professional',
                  );
                },
                ...
                PopupMenuButton<String>(
                  ...
                  onSelected: (value) async {
                    ...
                    if (value == 'report') {
                      ...
                      if (currentUserId == null || professional.id == null) return;
                      try {
                        await context.read<UserProvider>().unblockUser(currentUserId, professional.id!);
                        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('User unblocked')));
                      } catch (e) {
                        if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to unblock user: $e')));
                      }
                    }
                  },
                  itemBuilder: (context) => [
                  ...
                  children: [
                    ...
                    if (isBlocked)
                      Padding(
                        ...
                        child: Text(
                          iBlockedThem
                              ? "You've blocked this user. Unblock them to hire or message."
