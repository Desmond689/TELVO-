...
class AuthProvider extends ChangeNotifier {
        ...
        if (doc.exists) {
            ...
            try {
              ...
              _userDocSub = _firestore.collection('users').doc(userId).snapshots().listen((snapshot) async {
                ...
                if (d == null) return;
                if (d['isSuspended'] == true || d['isBanned'] == true) {
  ...
  Future<void> updateProfile(Map<String, dynamic> data) async {
    try {
      ...
      final verificationStatus = data.containsKey('verificationStatus')
          ? data['verificationStatus'] as String?
          : _currentUser!.verificationStatus;
      final verificationDeadline = data.containsKey('verificationDeadline')
          ? data['verificationDeadline'] as DateTime?
          : _currentUser!.verificationDeadline;
      final verificationSubmittedAt = data.containsKey('verificationSubmittedAt')
          ? data['verificationSubmittedAt'] as DateTime?
          : _currentUser!.verificationSubmittedAt;
      final verificationRejectedReason = data.containsKey('verificationRejectedReason')
          ? data['verificationRejectedReason'] as String?
          : _currentUser!.verificationRejectedReason;
  ...
  Future<String?> uploadFile({
    ...
    try {
      ...
      throw Exception('Upload failed. Could not upload to Cloudinary.');
    } catch (e) {
