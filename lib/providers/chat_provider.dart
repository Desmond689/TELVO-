...
class ChatProvider extends ChangeNotifier {
  ...
  Future<void> sendMessage(ChatMessage message) async {
    try {
      ...
      // Block check: either participant blocked the other
      ...
      if (senderId != null && receiverId != null) {
        ...
        final senderBlocked = List<String>.from(
          (senderDoc.data()?['blockedUsers'] as List?)?.map((e) => e.toString()) ?? [],
        );
        final receiverBlocked = List<String>.from(
          (receiverDoc.data()?['blockedUsers'] as List?)?.map((e) => e.toString()) ?? [],
        );
        if (senderBlocked.contains(receiverId) || receiverBlocked.contains(senderId)) {
