...
class JobProvider extends ChangeNotifier {
  ...
  Future<void> submitReview(ReviewModel review) async {
    try {
      // Only completed jobs can be reviewed; one review per job
      if (review.jobId == null || review.jobId!.isEmpty) {
        _setError('Missing job for review.');
        return;
      }
      final jobDoc = await _firestore.collection('jobs').doc(review.jobId).get();
      if (!jobDoc.exists) {
        _setError('Job not found.');
        return;
      }
      final jobStatus = (jobDoc.data()?['status'] as String?)?.toLowerCase() ?? '';
      if (jobStatus != 'completed' && jobStatus != 'reviewed') {
        _setError('You can only review completed jobs.');
        return;
      }
      final existing = await _firestore
          .collection('reviews')
          ...
          .get();
      if (existing.docs.isNotEmpty) {
        _setError('You already reviewed this job.');
        return;
      }
